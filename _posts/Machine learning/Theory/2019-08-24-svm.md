...
- **SVM은 scaling이 필요**
각 feature의 scale의 차이가 크다면 margin이 작아질 수 있기 때문에 **scaling이 필요**하지만, 항상 그렇듯이 주어진 문제에 따라 적절히 적용하는 것이 바람직합니다. <br>
...
Misclassification panelty. 작아질수록 도로의 폭이 넓어지지만, 도로 중간이나 심지어 반대쪽에 있는 샘플들의 개수가 많아집니다. (margin violation(마진 오류)이 증가) <br>
특히, overfitting이 발생한 경우 `C`를 감소시킬 수 있습니다. (`C` $ = \frac{1}{\lambda}$)

- **`SVC(kernel='linear', C=1)` 대신 `LinearSVC(C=1, loss='hinge')` 또는 `SGDClassifier(loss='hinge', alpha=1/(m*C))`를 사용**

- **`LinearSVC()`는 bias를 포함하여 regularization를 수행하기 때문에 scaling이 꼭 필요합니다!**

- **M > N 인 경우, `LinearSVC(dual=False, C=1)`**
...
plt.show()
```
![png](/images/svm_files/poly1.png)

```python
poly_kernel_svm_clf.set_params(poly_features__degree=3).fit(X, y)

plot_dataset(X, y, [-1.5, 2.5, -1, 1.5])
plot_predictions(polynomial_svm_clf, [-1.5, 2.5, -1, 1.5])
plt.show()
```
![png](/images/svm_files/poly2.png)
