...
$ \quad \quad \textbf{\color{red}E-step: } \ \hat{q}_i(h_i) ← Pr(h_i \mid x_i, \theta) = \frac{Pr(x_i \mid h_i, \theta) Pr(h_i)}{Pr(x_i \mid \theta)} $
$ \quad \quad \textbf{\color{blue}M-step: } \ \hat{\theta} ← \argmax_\theta \Sigma_i \int \hat{q}_i(h_i) \log Pr(x_i, h_i \mid \theta) dh_i$
