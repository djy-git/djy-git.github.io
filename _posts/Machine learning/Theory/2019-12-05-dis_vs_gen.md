...
| **Inference algorithm**  | Evaluate model $Pr(Y \mid X=x^\ast)$         | <ol><li> Define *prior* $Pr(Y)$ </li> <li> $Pr(Y \mid X=x^\ast)=\frac{Pr(X=x^\ast \mid Y) Pr(Y)}{\int Pr(X=x^\ast \mid Y) Pr(Y) dY}$ </li> |
