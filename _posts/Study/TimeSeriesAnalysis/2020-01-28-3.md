...
### - AR(1)
$x_t = \phi_1 x_{t-1} + a_t \quad \cdots \quad$ stationary if $\mid \phi_1 \mid < 1$ <br>
양변에 $x_{t-h}$를 곱해주고 expectation을 취하는 Y-K equation을 만들어주면, <br>

$\rho(h) = \phi_1 \rho(h-1), \ h \geq 1$ <br>
즉, $\rho(h) = \phi_1^h$ 이다. <br>

![png](/images/2020-01-28-3/1.png)

만약 data로부터 구한 $\rho(h)$가 위의 그림과 같이 $h$에 exponential한 관계를 보인다면, 이것은 AR(1) model을 사용하는 것이 좋다는 clue가 된다. <br>
