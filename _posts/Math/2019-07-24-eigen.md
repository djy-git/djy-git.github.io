...
<br>
#### Characteristics
$\mathbf{\lambda}: \text{Eigenvalue of } A \\ \mathbf{p, q}: \text{Eigenvector of } A$
...
\end{pmatrix}
$$
$\text{Eigenvalues are } \lambda, \mu, \nu \text{ and eigenvectors are } [\mathbf{p}, \mathbf{o}, \mathbf{o}]^T, [\mathbf{o}, \mathbf{q}, \mathbf{o}]^T, [\mathbf{o}, \mathbf{o}, \mathbf{r}]^T$
10. $\text{For triangular matrix, diagonal elements are eigenvalues}$
11. $ \text{For regular matrix S, } S^{-1} \mathbf{p} \text{ is eigenvector of } S^{-1}AS $
